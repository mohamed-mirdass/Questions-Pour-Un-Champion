...
function startTournament() {
  ...
  if (!TRN.questions.length) {
    toast('Aucune question disponible pour ce thème/niveau.', 'error', 2500);
    return;
  }
...
function trnLoadQuestion() {
  ...
  if (!q) {
    // Cas limite : plus de question disponible (pool épuisé) — on termine le match plutôt que de planter
    trnEndMatch();
    return;
  }
  m.activePlayer = null;
...
function trnShowFinalStandings() {
  ...
  document.getElementById('trnFullStats').innerHTML = `
        ...
        ${standings.map((idx, rank) => {
          ...
          return `
            <tr>
              ...
              <td style="color:var(--gold)">${trnBestMatchScore(idx)}</td>
            </tr>
          `;
        }).join('')}
      </tbody>
    ...
    <div class="trn-stats-title" style="margin-top:18px;">Détail par match</div>
    <table class="trn-stats-table card">
      <thead>
        <tr>
          <th>Match</th>
      ...
      <tbody>
        ${TRN.matches.filter(m => m.done).map(m => `
          <tr>
            <td>${m.emoji} ${m.label}</td>
            <td>${p[m.p1] ? p[m.p1].name : '—'}</td>
            <td style="color:var(--accent)">${m.score1}</td>
            <td>${p[m.p2] ? p[m.p2].name : '—'}</td>
            <td style="color:var(--accent)">${m.score2}</td>
            <td style="color:var(--gold)">${m.winner !== null && p[m.winner] ? p[m.winner].name : '—'}</td>
          </tr>
        `).join('')}
      </tbody>
    </table>
...
/* ────────────────────────────────────────────
   HELPERS
──────────────────────────────────────────── */
function trnBestMatchScore(pIdx) {
  let best = 0;
  TRN.matches.forEach(m => {
    if (m.p1 === pIdx) best = Math.max(best, m.score1 || 0);
    if (m.p2 === pIdx) best = Math.max(best, m.score2 || 0);
  });
  return best;
}
